...
describe("TimeLockedBackup core functions", function () {
  ...
  it("allows the user to create deploy the contract when constraints are met", async () => {
    ...
    await registry
      ...
      .set({
        notValidBefore: now,
        notValidAfter: now + 1000,
        recipient: userB.address,
      });
    await new TimeLockedBackup__factory(userA).deploy(
      userB.address,
      now,
      now + 1000,
      registry.address,
    ...
    const ethBalanceUserBAfter = await ethers.provider.getBalance(
      userB.address
    );
    expect(ethBalanceUserBAfter).to.equal(
      ethBalanceUserB.add(1000000),
      "userB should have received the funds from userA"
    );
  });

  it("should fail if the time constraints are not met", async () => {
    const tx = new TimeLockedBackup__factory(userA).deploy(
      userB.address,
      now + 9999999,
      now + 99999999,
      registry.address,
  ...
  it("should default to the constructor params if params are not set in the registry", async () => {
    ...
    expect(deployerBalanceAfter).to.equal(
      deployerBalance.add(1000000),
      "deployer should have received the funds"
    );
  });
  ...
  it("should override the default parameters if set in the registry", async () => {
    ...
    expect(deployerBalance).to.equal(
      ...
      "deployer balance should be unchanged"
    );
    expect(ethBalanceUserBAfter).to.equal(
      ethBalanceUserB.add(1000000),
      "userB should have received the funds from userA"
